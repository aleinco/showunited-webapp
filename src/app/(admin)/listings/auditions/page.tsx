...
export default function AuditionsAdminPage() {
  ...
  const { data, isLoading, isError } = useQuery({
    ...
    queryFn: async () => (await fetch('/api/admin/auditions-list')).json(),
    staleTime: 60_000,
  });
  const allRows = useMemo(() => data?.auditions || [], [data]);
  ...
  const onClose = useCallback(async (row: any) => {
    ...
    const res = await fetch('/api/admin/audition-status', {
      method: 'POST', headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ id: row.id, newStatusId: 10 }),
    });
    if (res.ok) { toast.success('Closed'); qc.invalidateQueries({ queryKey: ['admin-auditions'] }); }
  ...
  const onDelete = useCallback(async (row: any) => {
    ...
    const res = await fetch('/api/admin/delete-audition', {
      method: 'POST', headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ id: row.id }),
    });
    if (res.ok) { toast.success('Deleted'); qc.invalidateQueries({ queryKey: ['admin-auditions'] }); }
  ...
  return (
    <div className="space-y-4 p-4 md:p-6">
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <Title as="h4" className="text-lg font-semibold">Auditions</Title>
          <Text className="mt-1 text-sm text-gray-500">
            {isLoading ? 'Loading...' : `${totalCount} auditions`}
          </Text>
        </div>
        <div className="flex items-center gap-3">
          <Input type="text" placeholder="Search auditions..." value={searchInput}
          ...
          <Button size="sm" className="bg-primary text-white" onClick={() => router.push('/listings/auditions/create')}>Create Audition</Button>
        </div>
      </div>
          ...
          <table className="w-full">
            ...
            <tbody className="divide-y divide-muted">
              {isLoading ? (
                ...
                <tr><td colSpan={8} className="px-4 py-8 text-center text-sm text-gray-400">No auditions found</td></tr>
              ) : (
                paginatedRows.map((row: any) => (
                  <tr key={row.id} className={cn('bg-white transition-colors hover:bg-gray-50/50')}>
                    <td className="px-4 py-3.5">
                      <button onClick={() => router.push(`/listings/auditions/${row.id}`)} className="text-left hover:opacity-80">
                    ...
                    <td className="whitespace-nowrap px-4 py-3.5">
                      <div className="flex items-center justify-end gap-2">
                        {row.isOurs && (
                          <ActionIcon size="sm" variant="outline" aria-label="Edit" onClick={() => router.push(`/listings/auditions/${row.id}`)}>
        ...
        {paginatedRows.length > 0 && (
          <div className="flex flex-col gap-3 border-t border-muted px-4 py-3 sm:flex-row sm:items-center sm:justify-between">
            <Text className="text-sm text-gray-500">
              Showing {(page - 1) * pageSize + 1}–{Math.min(page * pageSize, totalCount)} of {totalCount} auditions
            </Text>
            <div className="flex items-center gap-3">
