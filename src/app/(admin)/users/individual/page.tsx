...
export default function IndividualUsersPage() {
  ...
  const allSelected = paginatedRows.length > 0 && paginatedRows.every((r) => selectedIds.has(r.id));
  ...
  const handleExportCSV = useCallback(() => {
    const exportData = (selectedRows.length > 0 ? selectedRows : filtered).map((r) => ({
      Name: r.name,
      Email: r.email,
    ...
    exportToCSV(exportData, 'individual-users');
  ...
  return (
    <div className="space-y-4 p-4 md:p-6">
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <Title as="h4" className="text-lg font-semibold">Individual Users</Title>
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-muted bg-gray-100/40">
                ...
                <th className="px-4 py-3.5 text-left text-xs font-semibold uppercase tracking-wider text-gray-500">User</th>
            ...
            <tbody className="divide-y divide-muted">
              {isLoading ? (
                ...
                <tr><td colSpan={7} className="px-4 py-8 text-center text-sm text-red-default">Error loading data</td></tr>
                ...
                paginatedRows.map((row) => {
                  ...
                  return (
                    <tr
                      key={row.id}
                      className={cn('bg-white transition-colors hover:bg-gray-50/50', isSelected && 'bg-primary-lighter/30')}
                    >
                      <td className="px-4 py-3.5">
                        <Checkbox aria-label={`Select ${row.name}`} checked={isSelected} onChange={() => toggleRow(row.id)} />
                      </td>
                      <td className="px-4 py-3.5">
                        <button onClick={() => router.push(`/users/individual/${row.id}`)} className="text-left transition-opacity hover:opacity-80">
                          <AvatarCard
                            src={row.photo || ''}
                            name={row.name || '---'}
